# Stowe Agents v2 — Inter-Agent Collaboration Design

## Architecture

Six agents connected through Linear as the communication bus. Any agent can hand off to any other. Every agent reads all issue comments before acting, so handoff instructions from triager, reviewer, or any other agent are always picked up.

### Agent Roster

| Agent | Linear User | Model | Role |
|-------|-----------|-------|------|
| stowe-triager | `stowe-triager` | claude-sonnet-4-6 | Routes issues, classifies, decides next agent |
| stowe-researcher | `stowe-researcher` | claude-sonnet-4-6 | Researches topics, reports findings (was: story-writer) |
| stowe-planner | `stowe-planner` | claude-sonnet-4-6 | Creates implementation plans from well-defined issues |
| stowe-implementer | `stowe-implementer` | claude-sonnet-4-6 | Writes code, opens PRs |
| stowe-code-reviewer | `stowe-code-reviewer` | claude-sonnet-4-6 | Reviews PRs for quality |
| stowe-security-auditor | `stowe-security-auditor` | claude-opus-4-6 | Audits for security vulnerabilities |

### Handoff Protocol (shared section — included in every agent)
...
**MCPs:** Linear, GitHub (both always-allow)

```
You are the technical triager for Stowe, a business banking platform (Platform → Partner → Merchant multi-tenancy).
...
### 2. stowe-researcher (renamed from story-writer)

**Model:** claude-sonnet-4-6
**MCPs:** Linear (always-allow)

```
You are a technical researcher for Stowe, a business banking platform (Platform → Partner → Merchant multi-tenancy).
...
```
You are a technical planner for Stowe, a business banking platform (Platform → Partner → Merchant multi-tenancy).
...
```
You are an implementation agent for Stowe, a business banking platform.
...
### 5. stowe-code-reviewer

**Model:** claude-sonnet-4-6
**MCPs:** Linear, GitHub (both always-allow)

```
You are a senior code reviewer for Stowe, a business banking platform.
...
### 6. stowe-security-auditor

**Model:** claude-opus-4-6
**MCPs:** Linear, GitHub (both always-allow)

```
You are a security auditor for Stowe, a business banking platform handling real money.
...
### Example 1: New feature from rough idea
```
Human creates issue "Add Plaid balance refresh"
  → Triager: classifies as feature/medium, routes to Researcher
    → Researcher: investigates Plaid API, reports findings, routes to Planner
      → Planner: writes implementation plan, routes to Implementer
        → Implementer: codes it, opens PR, routes to Code Reviewer
          → Code Reviewer: approves, routes to Security Auditor (touches financial data)
            → Security Auditor: passes, assigns back to Human for merge
```

### Example 2: Bug with clear repro
```
Human creates issue "500 error on /api/v2/merchants when partner has no merchants"
  → Triager: classifies as bug/high, clear scope, routes directly to Implementer
    → Implementer: fixes null check, opens PR, routes to Code Reviewer
      → Code Reviewer: approves (no auth/money changes), assigns back to Human
```

### Example 3: Review sends back for changes
```
  → Code Reviewer: REQUEST_CHANGES — missing null check, wrong DTO mapping
    → Implementer: reads review comments, fixes, pushes, re-assigns to Code Reviewer
      → Code Reviewer: approves this time
```

### Example 4: Planner discovers issue is too vague
```
  → Planner: "Can't plan this — unclear whether we need real-time sync or batch. Sending back to researcher."
    → Researcher: investigates both approaches, reports tradeoffs
      → Planner: now can plan, proceeds normally
```
